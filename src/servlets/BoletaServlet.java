...
public class BoletaServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		VentaDTO ven = new VentaService().ultimaVentaXUsuario(usu.getCodigo());
		
		ArrayList<CarritoDTO> listado = new CarritoService().listarUltimaVenta(ven.getNumVenta(), usu.getCodigo());
		...
		try
		{
			...
			Paragraph titulo = new Paragraph("***BOLETA DE VENTA***", FontFactory.getFont("arial", 30, Font.BOLD, BaseColor.BLACK));
			titulo.setAlignment(Element.ALIGN_CENTER);
			documento.add(titulo);
			
			//DATOS DEL USUARIO
			documento.add(new Paragraph(" "));
			documento.add(new Paragraph("Fecha	: " + getFormatoFecha(ven.getFecha()), FontFactory.getFont("arial", 13, Font.BOLD, BaseColor.BLUE)));
			documento.add(new Paragraph("Cliente: " + usu.getNombre() + " " + usu.getApellido(), FontFactory.getFont("arial", 13, Font.BOLD, BaseColor.BLUE)));
			documento.add(new Paragraph(" "));
			
			//LISTA DE PRODUCTOS
			PdfPTable tabla = new PdfPTable(6);
			tabla.setWidthPercentage(100);
			float[] widths = {1.3f,1.1f,0.8f,0.7f,0.7f,0.9f};
	...
	{
		PdfPCell celdita = new PdfPCell (new Paragraph(descripcion,
						                FontFactory.getFont("arial",   // fuente
							            11,                            // tamaño
